...
def fnlist_foldright(xs, ini, fopr_func):
    if xs.ctag == 0:
        ...
        return fopr_func\
            (xs.arg1, fnlist_foldright(xs.arg2, ini, fopr_func))
    # end-of-(if-then-else)
...
def fnlist_streamize(xs):
    def helper(xs):
        if xs.ctag == 0:
            ...
            return \
                strcon_cons(xs.arg1, lambda: helper(xs.arg2))
...
def stream_make_map(fxs, fopr):
    def helper(fxs):
        ...
        if cxs.ctag == 0:
            ...
            return strcon_cons\
                (fopr(cxs.arg1), lambda: helper(cxs.arg2))
        # end-of-(if(cxs.ctag==0)-then-else)
